...
#define DEBUG_IN(func,file) dsk_debug(IN,func,(file),err);

#define DEBUG_OUT(func,file)  dsk_debug(OUT,func,(file),err);

#else

#define DEBUG_IN(func,fcb) 
#define DEBUG_OUT(func,fcb) 
...
DEBUG_IN(BUFCREATE,fcb)
DEBUG_OUT(BUFCREATE,fcb)
...
DEBUG_IN(BUFSTATUS,fcb)

   err = (*fcb->DMod->Status)(fcb, avail);

DEBUG_OUT(BUFSTATUS,fcb)
...
DEBUG_IN(BUFDELETE,fcb)

   err = (*fcb->DMod->Delete)(fcb);

DEBUG_OUT(BUFDELETE,fcb)
...
DEBUG_IN(BUFFORMAT,fcb)

  err = (*fcb->DMod->Format)(fcb);

DEBUG_OUT(BUFFORMAT,fcb)
...
DEBUG_IN(BUFRESTORE,fcb)

   err = (*fcb->DMod->Restore)(fcb, mode);

DEBUG_OUT(BUFRESTORE,fcb)
...
DEBUG_IN(BUFDIRECTORY,fcb)

  err = (*fcb->DMod->Directory)(fcb, callback);

DEBUG_OUT(BUFDIRECTORY,fcb)
...
DEBUG_IN(BUFOPEN,fcb)

   err = (*fcb->DMod->Open)(fcb);

DEBUG_OUT(BUFOPEN,fcb)
...
DEBUG_IN(BUFCLOSE,fcb)

   err = (*fcb->DMod->Close)(fcb);

DEBUG_OUT(BUFCLOSE,fcb)
...
DEBUG_IN(O_CLOSE,fcb)
 
   If (!fcb) Then      /* TS 19-06-92 */  
      *done = CLOSE_DONE;
DEBUG_OUT(O_CLOSE,fcb)
      return err;
   Endif
