 ...
 *  hosts and addresses
 */
int cSocketApi::GetHostName(char * name, unsigned long name_length)
{
   int rc = gethostname(name, name_length);
   return rc;
}

unsigned long cSocketApi::IPAddress(const char * host_name)
{
   struct hostent * e = gethostbyname(host_name);
   if (e != NULL) {
      return *(unsigned long*)e->h_addr; 
   } else {
      return 0;
   }
}

int cSocketApi::IPAddresses(const char * host_name,
                            unsigned long & a1,
                            unsigned long & a2,
                            unsigned long & a3,
                            unsigned long & a4)
{
   ...
   if (e != NULL) {
      ...
      while(e->h_addr_list[i]) {
         IN_ADDR addr = {0};
	      addr.s_addr = *((long*)e->h_addr_list[i]);
         switch (i) {
         case 0:
            a1 = addr.s_addr;
            break;
...
int cSocketApi::TranslateAddress(char * address, unsigned long address_length, unsigned long ip_addr)
{
   if (0 == address_length) {
      return -1;
   }
   ...
   char temp[0x100] = {0};
   int ip[4] = {0};
   ip[3] = (ip_addr & 0xff000000) >> 24; 
   ip[2] = (ip_addr & 0x00ff0000) >> 16; 
   ip[1] = (ip_addr & 0x0000ff00) >> 8; 
   ip[0] = ip_addr & 0x000000ff;
   _snprintf(temp, sizeof(temp), "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
   unsigned long length = strlen(temp);
   if (address_length <= length) {
      rc = -1;
      length = address_length - 1;
   }
   memcpy(address, temp, length);
   return rc;
}

unsigned long cSocketApi::TranslateAddress(const char * address)
{
         ...
         for (c2=c2_start; c2<=c2_end; c2++) {
            for (c3=c3_start; c3<=c3_end; c3++) {
               for (c4=c4_start; c4<=c4_end; c4++) {
                  ...
                  _snprintf(ip_addr, sizeof(ip_addr), "%d.%d.%d.%d", c1, c2, c3, c4);
