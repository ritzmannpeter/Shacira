...
cQtKeypadNumeric::cQtKeypadNumeric(QWidget* parent, const char* name, bool modal, WIDGET_FLAGS_TYPE fl)
#ifdef QT4
   : cQtKeypadNumericBase(parent, name, modal, fl ? fl : Qt::CustomizeWindowHint|Qt::WindowTitleHint),
#else
...
{
   // Dialog-Font wird als Default für smallFont genommen, bigFont x1.3
   QFont f = font();
   _keyboard_input.setSmallFont(f);
   f.setPointSize(f.pointSize()*13 / 10);
   _keyboard_input.setBigFont(f);

   _keyboard_input.setEchoWidget(UI()->_new);
...
#ifdef QT4
   if (cQtKeypadNumericBase::windowTitle().isEmpty() == false) {
      if (cQtKeypadNumericBase::windowTitle() == cQtKeypadNumericBase::objectName()) {
         setWindowTitle(" ");
      }
   } else {
      setWindowTitle(" ");
...
#ifdef QT4
   UI()->wStackLabelActual->setCurrentIndex(stack_index);
   UI()->wStackLabelNew->setCurrentIndex(stack_index);
   UI()->wStackMinLabel->setCurrentIndex(stack_index);
   UI()->wStackMaxLabel->setCurrentIndex(stack_index);
#else
