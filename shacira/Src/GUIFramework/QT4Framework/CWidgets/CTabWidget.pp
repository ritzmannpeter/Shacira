...
#ifndef _ctabwidget_h_
#define _ctabwidget_h_
...
class CGroupSelectButton;
class CPage;
class CWidgetStack;
class cTabs;

class _CWIDGET_EXPORT_ CTabWidget : public QTabWidget, public CWidgetBase  
{
...
public slots:
   ...
   void currentChangedSlot(int index); //Slot that is called when Tab selection changes
...
public:
   CTabWidget(QWidget * parent, const char * name = 0, WIDGET_FLAGS_TYPE f = 0);
   ~CTabWidget();
   void SetSelectButton(CGroupSelectButton * select_button);
   CGroupSelectButton * GetSelectButton();
   void ShowGroup();
   void AddPage(CPage * page);
   void ActivatePage(CPage * page);
   void ShowPage(CPage * page);
   void DisablePage(CPage * page);
   void HidePage(CPage * page);
   QTabBar * TabBar();
...
private:
   void Reset();
   QWidget * TabContents(CTabWidget * tab_widget, CPage * page);
   CPage * Page(QWidget * tab_contents);
   void PrepareTabs();
...
private:
   ...
   BOOL_T _LanguageChanged;
   BOOL_T _ScrollingEnabled;
};
