...
void cRubberBand::Paint(QPainter & painter)
{
   ...
   if (_IsShown) {
      ...
      painter.drawRect(_TempRectangle.normalized());
   } else {
      painter.setPen(Qt::green);
      painter.drawRect(_ActRectangle.normalized());
   }
#else
...
{
#ifdef QT4
   left = UnscaleX(_ActRectangle.normalized().left());
   top = UnscaleY(_ActRectangle.normalized().top());
   right = UnscaleX(_ActRectangle.normalized().right());
   bottom = UnscaleY(_ActRectangle.normalized().bottom());
#else
