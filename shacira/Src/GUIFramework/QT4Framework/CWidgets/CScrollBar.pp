...
class _CWIDGET_EXPORT_ CScrollBar : public QScrollBar, public CWidgetBase  
{
...
//      emit(CheckAppearanceSignal(refresh_type, _AppearanceState));
   ...
   virtual void NextLineRequested()
   {
      NextLine();
   }
   virtual void PrevLineRequested()
   {
      PrevLine();
   }
...
protected:
   ...
   {
      QScrollBar::focusInEvent(e);
   };
   virtual void focusOutEvent(QFocusEvent * e)
   {
      QScrollBar::focusOutEvent(e);
   };
   void keyReleaseEvent(QKeyEvent * e)
   {
      QScrollBar::keyReleaseEvent(e);
   };
...
public:
   CScrollBar(QWidget * parent = 0, const char * name = 0, WIDGET_FLAGS_TYPE f = 0);
   ~CScrollBar();
private:
   LONG_T _Value;
   LONG_T _MinValue;
