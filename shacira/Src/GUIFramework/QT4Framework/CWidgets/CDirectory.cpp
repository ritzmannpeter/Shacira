...
class FileItem : public ListViewItem
{
public:
   FileItem(ListView * parent, ULONG_T index, const QString & file) :
      ListViewItem(parent), _Index(index), _File(file), 
...
#ifdef QT4
   // implemented in class ListViewItem
#else
   // for every column
...
protected:
   ...
   virtual void paintCell ( QPainter * p, const QColorGroup & cg, int column, int width, int align )
   {
      if (isColored(column)) {
         ...
         ListViewItem::paintCell(p, mycg, column, width, align);
      } else {
         ListViewItem::paintCell(p, cg, column, width, align);
      }
   }
#endif
private:
...
CDirectory::CDirectory (QWidget * parent, const char * name, WIDGET_FLAGS_TYPE f)
...
void CDirectory::SearchPathChange(QString search_path)
{
   _SearchPath = search_path;
   RefreshDirectory();
}

void CDirectory::RefreshDirectory(QString full_file_name)
{
#ifndef QT_PLUGIN
WMETHOD_PROLOG
...
#ifdef QT4
      if ((qsLastChar.indexOf('/') != -1) || (qsLastChar.indexOf('\\') != -1)) {
#else
...
WMETHOD_PROLOG
   ...
   case 1:
   {
       ...
       if (!qcsNewSearchPath.isEmpty() && (qcsNewSearchPath != _SearchPath))
       {
          _SearchPath = value;
          _Refresh = true;
          Notify(VIEW_DATA);
...
{
   FileItem * item = NULL;
   QString file_path = append(path, file_name);
...
#ifdef QT4
      SHORT_T ExtPos = qsFileName.lastIndexOf('.', -1);
#else
