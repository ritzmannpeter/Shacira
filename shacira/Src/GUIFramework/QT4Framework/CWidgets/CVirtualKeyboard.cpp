...
CVirtualKeyboard::CVirtualKeyboard(QWidget * parent, const char * name, WIDGET_FLAGS_TYPE f)
#ifdef QT4
   : QFrame(parent, f),
#else
...
#ifdef QT4
   setPaletteBackgroundColor(_InputFrame, Qt::green);
   setPaletteBackgroundColor(_KeyboardFrame, Qt::red);
...
WMETHOD_PROLOG
#ifdef QT4
   return (_PrevValue != _TextEdit->toPlainText());
#else
...
WMETHOD_PROLOG
#ifdef QT4
   _PrevValue = _TextEdit->toPlainText();
#else
