...
/// PR 28.01.15 - introduced a page shift mechanism to get rid of problems with the page
///               backward functionality in case of hidden (darked) pages.
///
...
#ifdef USE_PAGE_SHIFT_REGISTER
///
/// a page shift register records visited pages organized similar
/// to a shift register
/// a page shift register contains up to PAGE_SHIFT_REGISTER_CAPACITY pages
/// 

#define PAGE_SHIFT_REGISTER_CAPACITY      6
...
public:
   ...
   {
      int i = 0;
      for (i=0; i<PAGE_SHIFT_REGISTER_CAPACITY; i++) {
         _page[i] = NULL;
      }
   };
   virtual ~PageShiftRegister()
   {
   };
   void SetLastPage(CPage * page)
   {
      /// record a page a s being visited
      /// shift already visited pages one position back
      int i = 0;
      for (i=PAGE_SHIFT_REGISTER_CAPACITY-1; i>0; i--) {
         _page[i] = _page[i-1];
      }
      _page[0] = page;
   };
   inline bool PageIsVisible(CPage * page)
   {
      CTabWidget * tab_widget = page->TabWidget();
      int page_index = tab_widget->indexOf(page);
      if (page_index == -1) {
         return false;
      } else {
         return true;
   ...
   CPage * LastPage()
   {
      /// returns the last visted page that is not
      /// hidden (access rights or dark function)
      int i = 0;
      for (i=0; i<PAGE_SHIFT_REGISTER_CAPACITY; i++) {
         CPage * page = _page[i];
         if (page != NULL) {
            if (PageIsVisible(page)) {
               ...
               if (i == (PAGE_SHIFT_REGISTER_CAPACITY - 1)) {
                  return NULL;
               } else {
                  page = _page[i+1];
                  return page;
...
private:
   CPage * _page[PAGE_SHIFT_REGISTER_CAPACITY];
};
#endif

#define DEFAULT_PAGECONTAINER_WIDTH 1024
#define DEFAULT_PAGECONTAINER_HEIGHT 768

qreal CAppFrame::_xDisplayScale = 1.0;
qreal CAppFrame::_yDisplayScale = 1.0;
int CAppFrame::_pageContainerWidth  = DEFAULT_PAGECONTAINER_WIDTH;
int CAppFrame::_pageContainerHeight = DEFAULT_PAGECONTAINER_HEIGHT;
...
WMETHOD_VOID_EPILOG
...
#ifdef QT4
   _TabBarBackgroundColorValue = CWidgetBase::paletteBackgroundColor();
   _TabBarForegroundColorValue = CWidgetBase::paletteForegroundColor();
#else
   ...
   _PageShiftRegister = NULL;
#ifdef USE_PAGE_SHIFT_REGISTER
   _PageShiftRegister = new PageShiftRegister;
#endif
...
static CPage * ActivePage(CTabWidget * tab) 
{
   CPage * page = NULL;
   if (tab != NULL) {
      QWidget * widget = tab->currentWidget();
      page = dynamic_cast<CPage*>(widget);
   }
   return page;
}
...
WMETHOD_PROLOG
   ...
   CPage * active_page = ActivePage(page->TabWidget());
   if (active_page != page) {
      /// if the page to select does not equal the active
      /// page of the corresponding tab widget, ignore the selection
      /// because every group selection emits a show event for the page
      /// in the first tab (??? not clarified why)
      /// and setting this page as last page will confuse the forward backward mechanism
      return;
   }
#endif
   ...
   CPage * last_page;
#ifdef USE_PAGE_SHIFT_REGISTER
   _PageShiftRegister->SetLastPage(_ActPage);
   _ActPage = page;
   last_page = _PageShiftRegister->LastPage();
#else
   _LastPage = _ActPage;
   _ActPage = page;
   last_page = _LastPage;
#endif
   PageChanged(last_page, _ActPage);
   if (last_page != _ActPage)
   {
      if (last_page)
         last_page->Leave();
...
{
#ifdef USE_PAGE_SHIFT_REGISTER
   return _PageShiftRegister->LastPage();
#else
   return _LastPage;
#endif
}
...
QString CAppFrame::StyleSheet(QWidget * widget)
{
   if (TOUCH_SCREEN_ACTIVE()) {
      if (widget->inherits("QComboBox")) {
         return "QComboBox::drop-down {border-width: 0px;}\
                 QComboBox::down-arrow {border-width: 0px;}";
      } else if (widget->inherits("QTabBar")) {
         QColor tabBarColor = getTabBarBackgroundColor();
         QString tabBarColorStr = QString("%1%2%3").arg(tabBarColor.red(), 2, 16, QChar('0')).arg(tabBarColor.green(), 2, 16, QChar('0')).arg(tabBarColor.blue(), 2, 16, QChar('0')).toUpper();
         QString qTabBarToolButtonStyle = QString("QTabBar QToolButton {background: #%1; border: 1px outset #C4C4C3; border-bottom-color: #C2C7CB;}").arg(tabBarColorStr);

         QString style = "QTabBar::scroller {width: 30px}" +
                         qTabBarToolButtonStyle +
                         "QTabWidget::pane{border-top: 1px outset #C2C7CB;}" +
                         "QTabBar::tab{margin-left: 1px; min-height: 22px; border: 1px outset #C4C4C3; border-bottom-color: #C2C7CB; border-top-left-radius: 4px; border-top-right-radius: 4px; min-width: 8ex; padding: 2px 10px 2px 10px;}" +
...
WMETHOD_PROLOG
#ifdef USE_PAGE_SHIFT_REGISTER
   CPage * last_page = _PageShiftRegister->LastPage();
   ShowPage(last_page);
#else
   if (_LastPage != NULL) {
      ShowPage(_LastPage);
...
WMETHOD_PROLOG
#ifdef USE_PAGE_SHIFT_REGISTER
   _PageShiftRegister->SetLastPage(page);
#else
   _LastPage = page;
#endif
WMETHOD_VOID_EPILOG
...
#ifndef QT_PLUGIN
WMETHOD_PROLOG
   QDialog * qDialog = Dialog(dialog_name);
   if (qDialog == NULL) {
      QWizard * qWizard = Wizard(dialog_name);
      if (qWizard != NULL) {
#ifdef QT4
         WizardBase * wizard = static_cast<WizardBase *>(qWizard);
         ...
         if (_ActWizard == NULL) {
            ...
            QPoint pt1App = mapToGlobal(QPoint(0, 0));
            QPoint pt1Board = qWizard->mapToGlobal(QPoint(0, 0));
            SHORT_T X = (getPageAreaSize().width() - qWizard->width()) / 2 + pt1App.x();
            SHORT_T Y = (getPageAreaSize().height() - qWizard->height()) / 2 + pt1App.y();
            QPoint ptDialog(X, Y);
#ifdef QT4
            if (wizard->getPositionExclusive()) {
               ptDialog = wizard->pos();
            }
#endif
            qWizard->move(ptDialog);
...
#ifdef QT4
            if (wizard->getPositionExclusive()) {
               wizard->setPositionExclusive(false);
            }
#endif
         }
         return rc;
      ...
      int rc = 0; //Rejected
#ifdef QT4
      DialogBase * dialog = static_cast<DialogBase *>(qDialog);
#endif
      if (_ActDialog == NULL) {
         SetWidgetActive(qDialog, true);
         SetActive(qDialog, true);

         QPoint pt1App = mapToGlobal(QPoint(0, 0));
         QPoint pt1Board = qDialog->mapToGlobal(QPoint(0, 0));
         SHORT_T X = (getPageAreaSize().width() - qDialog->width()) / 2 + pt1App.x();
         SHORT_T Y = (getPageAreaSize().height() - qDialog->height()) / 2 + pt1App.y();
         QPoint ptDialog(X, Y);
#ifdef QT4
         if (dialog->getPositionExclusive()) {
             ptDialog = dialog->pos();
         }
#endif
         qDialog->move(ptDialog);

         rc = qDialog->exec();
         SetActive(qDialog, false);
         SetWidgetActive(qDialog, false);
#ifdef QT4
         if (dialog->getPositionExclusive()) {
             dialog->setPositionExclusive(false);
         }
#endif
      }
      return rc;
...
QSize CAppFrame::getPageAreaSize() const 
{ 
	return _PageAreaSize.isValid() ? _PageAreaSize : this->size();
}

qreal CAppFrame::xDisplayScale()
{
    loadUiParameters();
    return _xDisplayScale;
}

void CAppFrame::setXDisplayScale(qreal scale)
{
    _xDisplayScale = qBound(qreal(1.0), scale, qreal(5.0));
}

qreal CAppFrame::yDisplayScale()
{
    loadUiParameters();
    return _yDisplayScale;
}

void CAppFrame::setYDisplayScale(qreal scale)
{
    _yDisplayScale = qBound(qreal(1.0), scale, qreal(5.0));
}

int CAppFrame::getPageContainerWidth()
{
    loadUiParameters();
    return _pageContainerWidth;
}

void CAppFrame::setPageContainerWidth(int pageContainerWidth)
{
    _pageContainerWidth = pageContainerWidth;
}

int CAppFrame::getPageContainerHeight()
{
    loadUiParameters();
    return _pageContainerHeight;
}

void CAppFrame::setPageContainerHeight(int pageContainerHeight)
{
    _pageContainerHeight = pageContainerHeight;
}

#include "plugin_log.h"
BOOL_T CAppFrame::loadUiParameters()
{
   BOOL_T bRet = false;

#if QT_VERSION >= QT_VERSION_CHECK(5,0,0)
   QFile paramFile;
   QString paramFileName = "ui.parameters";
   paramFile.setFileName(paramFileName);
   if (paramFile.exists() && paramFile.open(QIODevice::ReadOnly)) {
      bRet = true;
   } else {
      paramFileName = getenv("UI_PARAMETER_FILE");
      paramFile.setFileName(paramFileName);
      if (paramFile.exists() && paramFile.open(QIODevice::ReadOnly)) {
         bRet = true;
      }
      else {
         paramFileName = ":/mosaic/configurations/ui.parameters";
         paramFile.setFileName(paramFileName);
         if (paramFile.exists() && paramFile.open(QIODevice::ReadOnly)) {
            bRet = true;
         }
   ...
   if (bRet) {
      QFileInfo paramFileInfo = QFileInfo(paramFile);
      QString absParamFilePath = paramFileInfo.absolutePath();
      QByteArray lineBuffer = paramFile.readLine();
      PluginPrintf("Ui parameter file name %s / Absolute parameter file path %s\n", CONST_STRING(paramFile.fileName()), CONST_STRING(absParamFilePath));	  
      string line = lineBuffer.toStdString();
      int pos = line.find("xDisplayScale");
      PluginPrintf("line %s pos %d\n", line.c_str(), pos);
      if (pos == 0) {
         pos = line.find("=");
         if (pos > 0) {
            QString value = line.substr(pos+1, line.length()).c_str();
            qreal xDisplayScale = value.toDouble();
            setXDisplayScale(xDisplayScale);
         }
      }

      lineBuffer = paramFile.readLine();
      line = lineBuffer.toStdString();
      pos = line.find("yDisplayScale");
      PluginPrintf("line %s pos %d\n", line.c_str(), pos);
      if (pos == 0) {
         pos = line.find("=");
         if (pos > 0) {
            QString value = line.substr(pos+1, line.length()).c_str();
            qreal yDisplayScale = value.toDouble();
            setYDisplayScale(yDisplayScale);
         }
      }
      lineBuffer = paramFile.readLine();
      line = lineBuffer.toStdString();
      pos = line.find("pageContainerWidth");
      PluginPrintf("line %s", line.c_str());
      if (pos == 0) {
          pos = line.find("=");
          if (pos > 0) {
              QString value = line.substr(pos + 1, line.length()).c_str();
              int pageContainerWidth = value.toInt();
              setPageContainerWidth(pageContainerWidth);
          }
      }
      lineBuffer = paramFile.readLine();
      line = lineBuffer.toStdString();
      pos = line.find("pageContainerHeight");
      PluginPrintf("line %s", line.c_str());
      if (pos == 0) {
          pos = line.find("=");
          if (pos > 0) {
              QString value = line.substr(pos + 1, line.length()).c_str();
              int pageContainerHeight = value.toInt();
              setPageContainerHeight(pageContainerHeight);
          }
      }
   }
#endif
