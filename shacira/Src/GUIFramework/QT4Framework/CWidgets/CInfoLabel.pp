...
#ifndef _cinfolabel_h_
#define _cinfolabel_h_

#include "CWidgetBase.h"
#include <qlabel.h>

/**!
  * @brief Specific display widget derived from QLabel that displays
  * textual information, directly sent to the gui.
  * <p>
  * The information to be displayed is not the value of a CCS variable reference
  * (as in the most CCS aware widgets) but the text of a cInfo type event sent to the GUI.
  *
  */
class _CWIDGET_EXPORT_ CInfoLabel : public QLabel, public CWidgetBase  
{
...
public:
   ...
   PROPERTY_2i(int,InfoTypeFilter)
...
public:
   CInfoLabel(QWidget * parent = 0, const char * name = 0, WIDGET_FLAGS_TYPE f = 0);
   ~CInfoLabel();
...
private:
   void SetInfo(INFO_PTR info);
   QString GetTranslatedAlarmText(INFO_PTR info);
};
