...
CSimpleGraphic::CSimpleGraphic(QWidget * parent, const char * name, WIDGET_FLAGS_TYPE f) :
...
WMETHOD_PROLOG
   ...
   DOUBLE_T x_max_value = _XMaxValue - _XMinValue;
   DOUBLE_T y_max_value = _YMaxValue - _YMinValue;
   if (x_max_value != 0) x_stretch_factor = _CurveRectangle.width() / x_max_value;
   if (y_max_value != 0) y_stretch_factor = _CurveRectangle.height() / y_max_value;
   ...
   if (_ViewModeValue == Ramps) {
      ...
      for (i = 0; i<(ULONG_T)_XCount; i++) {
         if (_SystemTypeValue == RightBottom || _SystemTypeValue == RightTop) {
            ...
            temp_xvalues[i] = _XValues[i] - _XMinValue;
      ...
      point_array.resize(array_size);
      ULONG_T pi = 0;
      DOUBLE_T x_pixels, y_pixels;
      for (i = 0; i<array_size / 2; i++) {
         // Don't paint at zero or max because the pen width is 2 pixels
         x_pixels = _MIN_(temp_xvalues[i] * x_stretch_factor, _CurveRectangle.width() - 1);
         x_pixels = _MAX_(x_pixels, 1);
         points[pi].setX(x_pixels);
         y_pixels = _MIN_((_YMaxValue - temp_yvalues[i]) * y_stretch_factor, _CurveRectangle.height() - 1);
         y_pixels = _MAX_(y_pixels, 1);
         points[pi].setY(y_pixels);
         point_array.setPoint(pi, points[pi]);
         pi++;
         // the last x point is always the max value
         if (i == (array_size / 2) - 1)
            x_pixels = _MIN_(x_max_value * x_stretch_factor, _CurveRectangle.width() - 1);
         else
            x_pixels = _MIN_(temp_xvalues[i + 1] * x_stretch_factor, _CurveRectangle.width() - 1);
         x_pixels = _MAX_(x_pixels, 1);
         points[pi].setX(x_pixels);
         y_pixels = _MIN_((_YMaxValue - temp_yvalues[i]) * y_stretch_factor, _CurveRectangle.height() - 1);
         y_pixels = _MAX_(y_pixels, 1);
...
#ifdef QT4
   QMatrix matrix(1, 0, 0, 1, _CurveRectangle.left(), _CurveRectangle.top());
#else
   QWMatrix matrix(1, 0, 0, 1, _CurveRectangle.left(), _CurveRectangle.top());
#endif
