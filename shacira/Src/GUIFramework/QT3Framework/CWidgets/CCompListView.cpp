...
CCompListView::CCompListView(QWidget * parent, const char * name, WFlags f) :
...
QString CCompListView::VarRefText(const QString & varRefSpec)
{
    ...
    if (var_ref == NULL) {
        description = qApp->translate("custom", CONST_STRING(varRefSpec));
    } else {
...
WMETHOD_PROLOG
    ...
    while (g != group_names.cend()) {
       QListViewItem * node = new QListViewItem(this, 0);
       node->setOpen(TRUE);
       ...
       while (e != entries->cend()) {
          cCompEntry * entry = *e;
          if (entry != NULL) {
             QListViewItem * item = new QListViewItem(node, 0);
