...
ClickFilter * CWidgetBase::_ClickFilter = NULL;
...
WMETHOD_PROLOG
   ...
   if (_ClickFilter == NULL) {
      _ClickFilter = new ClickFilter;
   }
   if (_ClickFilter != NULL &&
       qApp != NULL) {
      qApp->installEventFilter(_ClickFilter);
   }
...
WMETHOD_PROLOG
   ...
   if (_QWidget->inherits("CWidgetStack")) {
      ...
      QString classname = _QWidget->className();
      UNEXPECTED_BRANCH()
   }
...
QWidget * CWidgetBase::CustomParent(QWidget * widget)
{
...
WMETHOD_PROLOG
   ...
   if (rc != actionReject) {
      if ((dialog_name.isEmpty() == false) &&
         ...
         if (app_frame != NULL) {
            CActionDialog * action_dialog = app_frame->GetActionDialog(dialog_name, NULL);
            if (action_dialog != NULL) {
               action_dialog->Reset();
               action_dialog->SetContext(_Node, _Context);
               action_dialog->SetFunction(this, button_func_ref);
               cPublicDialog * dialog = (cPublicDialog*)action_dialog->Dialog();
               if (dialog != NULL) {
...
#ifndef QT_PLUGIN
WMETHOD_PROLOG
   new_text = qApp->translate(CONST_STRING(context), CONST_STRING(new_text));
...
WMETHOD_PROLOG
   if (!text.isEmpty()) {
      ...
      int i = 0;
      for (i=0; i<len; i++) {
         unsigned char c = ((unsigned char)text[i] >> 4) & 0x0f;
         if (c > 7) {
            if (i >= len) {
               // dereferencing i+1 in this case may crash the application
               return QString::fromUtf8((CONST_STRING_T)text);
            }
            if (c == 0x0c || c == 0x0d) {
               // first of 2 octet
