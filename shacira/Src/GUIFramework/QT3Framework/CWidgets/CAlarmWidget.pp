...
class CAlarmWidgetItem : public QListViewItem
{
public:
   CAlarmWidgetItem(QListView * parent, const QString & help_id);
...
/**!
  * @brief A widget to display alarms tahat are hold by the CCS service.
  * as a list with columns for time, severity, alarm text, ...
  * <p>
  */
class SH_QT_WIDGET_PLUGIN_EXPORT CAlarmWidget : public QListView, public CWidgetBase  
{
...
public:
   CAlarmWidget(QWidget * parent = 0, const char * name = 0, WFlags f = 0);
   ~CAlarmWidget();

protected:
   void ShowAlarms();
   void UpdateWidget();
   ULONG_T ItemPos(QListViewItem * item);
protected:
   bool _Invalidated;
