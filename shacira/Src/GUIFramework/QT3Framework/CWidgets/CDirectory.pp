...
class SH_QT_WIDGET_PLUGIN_EXPORT CDirectory : public CListView
{
// start 2i generic widget definition
   ...
   PROPERTY_2i(QCString,Col0Ref)
   PROPERTY_2i(QCString,Col1Ref)
   PROPERTY_2i(QCString,Col2Ref)
   PROPERTY_2i(QCString,Col3Ref)
   PROPERTY_2i(QCString,Col4Ref)
   PROPERTY_2i(QCString,Col5Ref)
...
//      emit(CheckAppearanceSignal(refresh_type, _AppearanceState));
   ...
   void SearchPathChange(QCString search_path);
   void RefreshDirectory(QCString full_file_name = "");
   void RefreshDirectory(ULONG_T button_result);
   void SelectionChanged();
   void SlotClearSelection();

signals:
   void FileSelected(QCString path);

public:
   CDirectory(QWidget * parent = 0, const char * name = 0, WFlags f = 0);
   ~CDirectory();
...
private:
   ...
   QCString _SearchPath;           // path to search for files
   int m_iMaxNoOfPatternFiles;
   QListViewItem * _qlvWrittenFileItem;
   BOOL_T _Refresh;
