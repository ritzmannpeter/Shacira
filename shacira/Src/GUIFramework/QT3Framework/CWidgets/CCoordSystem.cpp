...
CCoordSystem::CCoordSystem(QWidget * parent, const char * name, WFlags f)
...
void CCoordSystem::CalculateScaleAreas()
{
   ULONG_T i = 0;
   ULONG_T x_pos = 0;
   ULONG_T x_steps = _MIN_(_XStepsValue + 0, MAX_SCALE_STEPS);
   ULONG_T x_size = _XAxisRectangle.width();
   ULONG_T x_diff = x_size / x_steps;
   ULONG_T y_pos = 0;
   ULONG_T y_steps = _MIN_(_YStepsValue + 0, MAX_SCALE_STEPS);
   ULONG_T y_size = _YAxisRectangle.height();
