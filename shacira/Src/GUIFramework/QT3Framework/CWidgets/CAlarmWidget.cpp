...
// CAlarmWidgetItem
CAlarmWidgetItem::CAlarmWidgetItem(QListView * parent, const QString & help_id)
   : QListViewItem(parent), _help_id(help_id)
{
};
...
CAlarmWidget::CAlarmWidget(QWidget * parent, const char * name, WFlags f) :
...
void CAlarmWidget::SelectionChanged(QListViewItem * item)
{
#ifndef QT_PLUGIN
WMETHOD_PROLOG
   if (item != NULL) {
   ...
   if (alarms > 0) {
      for (ULONG_T i=0; i<MAX_ALARMS; i++) {
         ...
         if (ident > 0) {
            ...
            switch (severity) {
            case SevInfo:
               item->setPixmap(0, _InfoImage);
               break;
            case SevWarning:
               item->setPixmap(0, _WarningImage);
               break;
            case SevError:
               item->setPixmap(0, _ErrorImage);
               break;
            case SevSeriousError:
               item->setPixmap(0, _SeriousErrorImage);
               break;
            }
...
ULONG_T CAlarmWidget::ItemPos(QListViewItem * item)
{
#ifndef QT_PLUGIN
   if (item != NULL) {
      ...
      QListViewItem * search_item = it.current();
