...
CActValue::CActValue(QWidget * parent, const char * name, WFlags f)
   : QLabel(parent, name),
     CWidgetBase(this), _Text("")
{
   ...
   _VarView = NULL;
   _FormatValue = Native;
   _TranslationValue = NoTranslation; ///< HA10022006
...
WMETHOD_PROLOG
   ...
   if (_FormatValue == HexaDecimal) {
      ...
      if (size == 1) {
         qValue.sprintf("%02.2X", lval);
      } else if (size == 2) {
         qValue.sprintf("%04.4X", lval);
      } else if (size == 4) {
         qValue.sprintf("%08.8X", lval);
      } else {
         qValue.sprintf("%X", lval);
      }
      setText(qValue);
         ...
         if (var_ref != NULL) {
            ...
            if (var_def != NULL) {
               ...
               if ((mapping != NULL) &&
                  ...
                  int pos = 0;
                  SHORT_T  param = 0;
                  QString qcsDimension = _VarSpec.right(_VarSpec.length() - _VarSpec.find('['));
