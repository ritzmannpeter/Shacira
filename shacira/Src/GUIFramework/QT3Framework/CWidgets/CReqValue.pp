...
class cQtKeypadNumeric;

class SH_QT_WIDGET_PLUGIN_EXPORT CReqValue : public QLineEdit, public CWidgetBase  
{
...
public:
   ...
   Q_ENUMS(ValidTypes)
   PROPERTY_2i(ValidTypes,Validation)
...
public:
   ...
   enum ValidTypes {
         DataTypeSpecific = 0, ///< data type specific unrestricted validation
         PathValidation = 1,   ///< accept only valid file system path syntax
         UserDefined = 2       ///< user defined validaton must be set with SetValidator
   };
   /// end of common property definitions of C widgets

   // further widget specific properties
   PROPERTY_2i(InputModes,InputMode)
   VAR_REF_2i(Var)
...
public:
   enum InputModes {
         SingleLine = 0,       ///< single line editing when alphanumeric keyboard is used
         MultiLine = 1         ///< multi line editing when alphanumeric keyboard is used
   };
...
signals:
   ...
   void ValueAccepted(QCString var_ref, QString value);
...
public:
   CReqValue(QWidget * parent = 0, const char * name = 0, WFlags f = 0);
   ~CReqValue();
...
protected:
   ...
   void CancelInput();
   void ApplyInput();
   void NumericInput(QValidator * syntax_validator);
   void AlphaNumericInput(QValidator * syntax_validator);

protected:
   QString _Text;
   QString _PrevValue;
   cInput _Input;
   static cQtKeypadNumeric * _NumericKeypad;
   static cQtKeyboardAlpha * _EnglishKeyboard;
   static cQtKeyboardAlpha * _GermanKeyboard;
   QValidator * _UserValidator;
   QValidator * _PathValidator;
};
