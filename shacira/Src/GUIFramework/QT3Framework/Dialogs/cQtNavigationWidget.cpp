...
cQtNavigationWidget::cQtNavigationWidget(CAppFrame * parent,  
                                         ...
                                         ULONG_T flags,
                                         QFont qfont)
   : QWidget(parent, name, f),_FontPointSizeOffset(0),_FontPixelSizeOffset(0)
{
...
void cQtNavigationWidget::Calculate()
{
   ...
   int size = _Groups.size();
   int height = _AppFrame->height();
   int group_height = height / size;
   int group_width = group_height;
   int start = 0;
   _GroupWidth = group_height;
   if (maxPages > 0) {
      _GlobalPageWidth = (_AppFrame->width() - _GroupWidth) / maxPages;
   } else {
   ...
   for (int i=0; i<size; i++) {
      ...
      if (group_desc != NULL) {
         ...
         if (group_desc->GetRight()) {
            group_rect.setX(_AppFrame->width() - group_width);
