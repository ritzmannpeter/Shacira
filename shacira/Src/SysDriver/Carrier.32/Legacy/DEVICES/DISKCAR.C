...
#ifdef QNX_BASED
LOCAL void dsk_debug (BYTE type, int func, FCB_PTR fcb, int err)
{
    If (type == IN) Then
...
#else
LOCAL void dsk_debug (BYTE type, int func, int err, int order, USR_WORD offs, USR_WORD len)
{
    If (type == IN) Then
       printf("entry %s order %d pos %d len %d\n",
               dtab[func].name, order, offs, len);
    Else
		 printf("exit %s %4.4d\n",dtab[func].name, err);
    Endif
    return;
...
#define DEBUG_IN(func,order,offs,n) dsk_debug(IN,func,0,order,offs,n);
#define DEBUG_OUT(func,err)  dsk_debug(OUT,func,err,0,0,0);

#else

#define DEBUG_IN(func,order,offs,n)
#define DEBUG_OUT(func,err)
...
DEBUG_IN(CARRIERIO,order,offs,n)

   Switch ( order )
   Case 0:
/* 25.03.96
    status = DATACAR_Read(1, buf, offs, n);
*/
    status = BASIC_Read(offs, n, buf);
    If (status == DCAR_OK) Then
         status = USR_NO_ERROR;
      Else
         DiagPutMsg(status, (long)DATACAR_READ, 0, __FILE__, __LINE__ ); 
      Endif
      break;
   Case 1:
      For (status=-1; retries && (status < 0); retries--) Do
/* 25.03.96
			status = DATACAR_Write(1, offs, buf, n);
*/
         status = BASIC_Write(offs, n, buf);
         If (status == DCAR_OK) Then
            status = USR_NO_ERROR;
         Else
            DiagPutMsg(status, (long)DATACAR_WRITE, 0, __FILE__, __LINE__ ); 
         Endif
      Endfor
      break;
    Case SELECT_CARRIER:
...
*/
      ...
      Else
         *buf = 0;
         DiagPutMsg(status, (long)DATACAR_CHECK, 0, __FILE__, __LINE__ ); 
      Endif
      break;
   Endswitch

#define  DOSVERSION_ERROR_HANDLING
#ifdef  DOSVERSION_ERROR_HANDLING
	Switch (status)
      Case      DCAR_BAD_TYP:
...
#else
   Switch (status)
      Case DATACAR_INVALID_ADDRESS_OR_SIZE:
                Case CANMEM_INVALID_ADDRESS:
                Case CANMEM_INVALID_ADDRESS_OR_SIZE:
         status = CERR_ADDRESS;
         break;
      Case DATACAR_NOT_DETECTED:
		Case CANMEM_NO_CARRIER_DETECTED:
         status = CERR_NO_DEVICE;
         break;
      Case DATACAR_VERIFY_ERROR:
                Case CANMEM_WRITE_ERROR:
         status = CERR_WRITE_FAULT;
         break;
/*
      Case      DCAR_BAD_TYP:
         DiagPutMsg( status, (long) status, 0, __FILE__, __LINE__ ); 
         status = CERR_WRONG_FORMAT;
         break;
      Case      DCAR_NOT_FOUND:
         DiagPutMsg( status, (long) status, 0, __FILE__, __LINE__ ); 
      ...
      Case      DCAR_NOT_READY  :
         DiagPutMsg( status, (long) status, 0, __FILE__, __LINE__ ); 
         status = CERR_HARDWARE;
         break;
		Case      DCAR_ADR_ERROR  :
         DiagPutMsg( status, (((long)offs<<16) | (long)n) , 0, __FILE__, __LINE__ ); 
         status = CERR_ADDRESS;
         break;
*/
   Endswitch
...
STATIC int SearchFile1p( PARTINFO * ppinfo , char * intfn )
{
   ...
   For ( i=0 ; i<ppinfo->DirEntries ; i++ ) Do
      ...
      If ( first_free==0 && FileDeleted(dir.Name[0]) ) Then
         first_free = -(16+i*sizeof(DIRENTRY));
