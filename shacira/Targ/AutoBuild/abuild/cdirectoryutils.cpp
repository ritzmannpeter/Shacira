...
const char * cDirectoryUtils::_PathDelimiter = SLASH_STRING;
...
#endif
   while (stricmp(left_dir.c_str(), left_path.c_str()) == 0) {
      if (left_dir.size() == 0) {
         return "./";
      }
      _dir = cDirectoryUtils::LeftStrip(_dir.c_str(), left_dir);
...
bool cDirectoryUtils::CopyFile(const char * src_path, const char * dst_path)
{
   unsigned long size = FileSize(src_path);
   char * buffer = (char*)__ALLOC__(size);
   FILE * stream = fopen(src_path, "r");
   if (stream != NULL) {
      fread(buffer, size, 1, stream);
      fclose(stream);
   }
   stream = fopen(dst_path, "w");
   if (stream != NULL) {
