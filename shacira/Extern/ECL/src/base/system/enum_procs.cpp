...
EnumProc.c (or .cpp)
*********************/ 
#include "enum_procs.h"
#include <tlhelp32.h>
#ifdef __MINGW32__
// PR GNUC
// vdmdbg.h is not part of mingw
...
BOOL WINAPI EnumProcs( PROCENUMPROC lpProc, LPARAM lParam )
{
  ...
  if( osver.dwPlatformId == VER_PLATFORM_WIN32_NT )
  {
     ...
     if( lpfEnumProcesses == 0 ||
        lpfEnumProcessModules == 0 ||
        lpfGetModuleFileNameEx == 0 ||
        lpfVDMEnumTaskWOWEx == 0)
        {
  ...
  }else if( osver.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS )
  {
     ...
     if( lpfProcess32Next == 0 ||
        lpfProcess32First == 0 ||
        lpfCreateToolhelp32Snapshot == 0 )
     {
        FreeLibrary( hInstLib ) ;
        return FALSE ;
     }
