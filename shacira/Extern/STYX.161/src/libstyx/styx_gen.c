...
{ Styx_T lanctx, rootctx; bool startflg = False; int indcnt = 0, rc;
  ...
  { PT_Term t = PT_termIT(it), st = (PT_Term)NULL;
    ...
    { styxQlxDfn qd; styxDfn pd; styxPrd p; GLS_Tok id, sgid, dgid;
      ...
      if (styx_QlxDfn(t,&qd) )
      { styxQlxOpt o; styxQlxCat c; styxQlxGrp gsrc, gdst; styxExp e;
        ...
        if ( styxQlxDfn_defn(qd,&c,&o,&gsrc,&id,&gdst,&e) )
        {
          ...
          else 
          {
            ...
            if( styxQlxCat_lanC(c) )
            {
              if( !(styxQlxGrp_grp(gsrc,&sgid) && styxQlxGrp_grp(gdst,&dgid)) )
                PT_diag_err
                (
                  qd,styctx->diag,"incomplete embedded language specification"
                );
              else
            ...
            { symbol srcsym = (symbol)NULL,
              ...
              if( MAP_defined(rootctx->embed,srcsym) )
                PT_diag_err(gsrc,styctx->diag,"no regular group");
              ...
              {
                MAP_dfndom(styctx->tokflg,id,SCN_FLG_SwitchToken);
                ...
                if( ! styxQlxCat_tokC(c) && ! styxQlxCat_comC(c) )
                  PT_diag_err(c,styctx->diag,"not allowed switch token category");
              }
            }
          }
          ...
          /* dyck nicht bei Kategorien let, lan, ind */
          if( styxExp_dyck(e,_,_,_) && 
              (styxQlxCat_letC(c) || styxQlxCat_lanC(c) || styxQlxCat_indC(c)) )
            PT_diag_err(e,styctx->diag,"dyck token definition not allowed here");
          /* maximal 1 (de)indent token pro scanner (group) */
      ...
      if (styx_Exp(t,&exp) )
      {
        if ( styxExp_ident(exp,&id) )
        {
          ...
          if (!MAP_defined(tokctx->glo,id))
            ...
            if( qlx_usage(tokctx,id) == UsageLan || qlx_dyck(tokctx,id) )
              PT_diag_err
              (
...
{ Styx_T rootctx = RootCtx(styctx),
  ...
  { styxQlxGrp gsrc; symbol grpsym;
    ...
    { styxQlxCat c;
      ...
      if( ! styxQlxCat_tokC(c) && ! styxQlxCat_comC(c) )
        PT_diag_err(c,parctx->diag,"not allowed EOF or follow token category");
    }
  }
...
{ GLS_Tok id; MAPIT itr; RegSet_T a, u, d; symbol rng; byte f;
  ...
  {
    if( rng == grpsym )
    { styxQlxDfn qd; styxQlxGrp gsrc, gdst; styxQlxOpt o;
      qd = MAP_apply(styxQlxDfn,styctx->glo,id);
      if( styxQlxDfn_defn(qd,_,&o,&gsrc,&id,&gdst,_) )
      { 
  ...
  if( ftoks != (MAP(_,_))NULL )
  {
    MAP_forItrAsg(id,a,itr,ftoks)
    {
      if( a != (RegSet_T)NULL )
      {
        ...
        Scn_dfnToken_reentrant
        (
          scndfn, symbolToString(GLS_Tok_symbol(id)),f,a
        );
      }
    }
  }
...
{ Styx_T rootctx = RootCtx(styctx), lanctx;
  ...
  if( !MAP_defined(grpset,stringToSymbol(igrpid)) )
  {
    ...
    trans_QlxGroupDfns(styctx,scndfn,InitialGroup,ftoks);
  ...
  { 
    if( !MAP_defined(rootctx->embed,grpsym) )
    {
      if( grpsym != InitialGroup )
      {
        ...
        if( !MAP_defined(grpset,stringToSymbol(grpid)) )
        {
          ...
          Scn_setMsgFun_reentrant(scndfn,PT_diag_msgFun(parctx->diag));
          trans_QlxGroupDfns(styctx,scndfn,grpsym,ftoks);
    ...
    { 
      ...
      trans_QlxGroups_aux(lanctx, parctx, grpsym, g_ftoks, grpset);
    }
  }
  ...
  MAP_forItrAsg(id,rng,itr,styctx->tokgrp)
  {
    ...
    ScnGrp_dfnSwitch
    (
      parctx->Scn, grpid, GLS_Tok_string(id), sgrpid
    );
...
{ MAP(symbol,MAP(GLS_Tok,byte)) g_ftoks; 
  ...
  if( PT_diag_errorCnt(styctx->diag) == 0 )
  {
    ...
    trans_QlxGroups_aux(styctx,styctx,(symbol)NULL,g_ftoks,grpset);
...
{ PT_Itr it; GLS_Lst(styxQlxDfn) qlxdfns;
  ...
  { 
    PT_FORALL(it,src) 
    {
      if (PT_stateIT(it) == PT_POST) /* alle reservierten Worte */
...
void STYX_exportTab(Styx_T styctx, c_string Environ)
/* exports scan table definition 
...
*/
{
  ...
  if( styctx->Scn != (Scn_T)NULL )
    Scn_Src_Modul(Environ,styctx->Scn);
  if( styctx->PTab != (PLR_Tab)NULL )
    PLR_Src_Modul(Environ,styctx->PTab);
}
