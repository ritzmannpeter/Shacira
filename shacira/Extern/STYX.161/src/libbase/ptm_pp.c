...
#define COLS(l)     ( (short)(((unsigned long)(l)) & 0xFFFFUL) )
...
{ MAP(symbol,int) map;
  BUG_NULL(ctx->tkmmap);
  assert0(sep >= PTP_SEP_NON && sep <= PTP_SEP_ROW,"invalid seperator");
...
{ unsigned long l, r, c;
  ...
  r = r & 0xFFFF0000UL;
  c = ((unsigned long)col) & 0x0000FFFFUL;
  l = r | c;
  return l;
}
  ...
  { 
    ...
    { MAP(symbol,int) map = 
      ...
      if( MAP_defined(map,PT_symbol(r)) )
        res = MAP_apply_short(map,PT_symbol(r));
    }
  }
  return res;
}
