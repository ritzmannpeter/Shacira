...
zip -r %PRJ% %PRJ%\* -x *.exe -x *.obj -x *.*~ -x *.vp? -x *.vtg -x *.pch -x *.idb -x *.pdb -x *.sbr -x *.ilk -x *.plg -x *.bsc -x *.ncb -x *.opt -x *.dll -x *.exp -x *.map -x *.aps -x *.res -x *.log -x *.00? -x *.o -x *.bpl -x */CVS/* -x */144MB/* -x *.il* -x */Debug/* -x */Release/*
