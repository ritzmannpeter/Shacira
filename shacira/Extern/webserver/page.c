...
static void DebugPrint(int mode, int perm, const char * lpath, const char * path, int fd)
{
   FILE * stream = fopen("open.txt", "a");
   if (stream != NULL) {
      fprintf(stream, "open mode %d perm %d\n", mode, perm);
      fprintf(stream, "lpath = %s\n", lpath);
...
#else
   fd = gopen(lpath, mode, perm);
//   DebugPrint(mode, perm, lpath, path, fd);
   return (wp->docfd = fd);
#endif
#endif /* WEBS_PAGE_ROM */
}
