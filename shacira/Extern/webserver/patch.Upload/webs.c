 ...
 */	
			...
			wp->isBinary = 0; //PATCH for binary upload: reset the binary flag
			break;
 ...
 */
			if(ContainsBinary(text, nbytes)) {
				wp->isBinary  = 1; //PATCH for binary upload: set the binary flag
			}
 ...
 */
               if (text != NULL)
               {
				   ...
				   if(wp->isBinary) { //PATCH for binary upload: handle query as binary
				       len = wp->lenBinary;
