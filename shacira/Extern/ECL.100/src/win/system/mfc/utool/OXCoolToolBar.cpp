...
{
	dc.DrawDragRect(rect,CSize(2,2),CRect(0,0,0,0),CSize(0,0),NULL,NULL);
}
	...
	if(nIndex<m_nBitmapButtons)
	{
		*pResult=TRUE;
		...
		{		
			...
			{
				// tool tip is after "\n" in the string
				LPTSTR pTipText=_tcschr(strText,_T('\n'));   
				if(pTipText!=NULL)
				{	
		...
		case CDDS_PREPAINT:
			{
				...
				int nCustomizedButton=GetCustomizedButton();
				if(nCustomizedButton!=-1 && IsCustomButton(nCustomizedButton))
				{
					CDC dc;
					dc.Attach(pCDRW->hdc);
					CRect rect;
					GetItemRect(nCustomizedButton,rect);
					DrawCustomizedButton(dc,rect);
					dc.Detach();
				}
		...
		case CDDS_ITEMPREPAINT:
			{
				*pResult=CDRF_NOTIFYPOSTPAINT;	// request for ITEMPOSTPAINT
				...
				if(!GetToolBarCtrl().IsButtonHidden(pCDRW->dwItemSpec) && IsFlat()) 
				{
					if(m_dwComCtlVersion>=_IE40_COMCTL_VERSION)
					{
		...
		case CDDS_ITEMPOSTPAINT:
			{
				...
				if(IsButtonCustomized(nIndex) && !IsCustomButton(nIndex))
				{
					CDC dc;
					dc.Attach(pCDRW->hdc);
					CRect rect;
								  ...
								  BOOL bProperties)
{
	...
	if(IsCustomizable(TRUE) || IsCustomizable(FALSE))
	{
		...
		ar.Close();
		int nBufferLength=memFile.GetLength();
		...
		int nCustomButtonCount=m_mapCustomButtons.GetCount();
		...
		arCustom.Close();
		nBufferLength=memFileCustom.GetLength();
		ASSERT(nBufferLength>0);
	...
	if(!m_bCustomButtonsStateSaved)
	{
		...
		CMemFile memFile;
		CArchive ar(&memFile,CArchive::store);
		int nButtonCount=m_mapAllCustomButtons.GetCount();
		ar<<(DWORD)nButtonCount;
...
int COXCoolToolBar::AddCustomButton(OXCUSTOMBUTTONDESCRIPTOR& descriptor)
{
	...
	if(!m_mapAllCustomButtons.Lookup(descriptor.m_nID,descriptorTest))
	{
		...
		return m_arrAllCustomButtonIDs.Add(descriptor.m_nID);
	}
	else
	{
...
BOOL COXCoolToolBar::ShowCustomButton(int nIndex)
{
	...
	if(bShowCustomButton)
	{
		...
		descriptor.m_pCBTemplate->CBGetWindow()->MoveWindow(rect,bIsVisible);
		descriptor.m_pCBTemplate->CBGetWindow()->ShowWindow(SW_SHOWNA);
	}	
	else
	{
		UINT nIDTest;
