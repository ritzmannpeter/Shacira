...
COXMenuBar::~COXMenuBar()
{
	for(int nIndex=m_arrCreatedPopupMenus.GetSize()-1; nIndex>=0; nIndex--)
	{
	...
	while(TRUE)
	{
		...
		if(nCommandID==-1)
		{
			...
			VERIFY(::InsertMenu(hMenu,nButtonIndex,
				MF_BYPOSITION|MF_POPUP|MF_STRING,(UINT)hSubMenu,sText)!=0);
			if(nFinish!=0)
			{
	...
	if(IsCustomizable(TRUE))
	{
		...
		if(m_mapChangedMenus.Lookup(m_hMenu,nIndex))
		{
...
#ifndef _UNICODE
			VERIFY(SaveMenuState(m_hMenu,(pDocument==NULL ? NULL :
			...
			if(pDocument!=NULL)
			{
				...
				VERIFY(SaveMenuState(m_hMenu,lpszUnicodeUniqueName));
...
void COXMenuBar::DisplayPopupMenu(int nMenuItem)
{
	...
	UINT nFlags=TPM_LEFTBUTTON|TPM_RIGHTBUTTON;
...
void COXMenuBar::DrawButtons(CDC& dc)
{
	...
	// close button
	nPushState=(m_pressedBtn==OX_MNU_CLOSEBTN) ? DFCS_PUSHED : 0;
	...
	if(!m_rectRestoreBtn.IsRectEmpty())
	{
		...
		nPushState=(m_pressedBtn==OX_MNU_RESTOREBTN) ? DFCS_PUSHED : 0;
	...
	if(!m_rectMinimizeBtn.IsRectEmpty())
	{
		...
		if((dwStyle&WS_MINIMIZEBOX)==0)
			nState|=DFCS_INACTIVE;
		nPushState=(m_pressedBtn==OX_MNU_MINIMIZEBTN) ? DFCS_PUSHED : 0;
...
int COXMenuBar::GetPrevMenuItem(int nItem, 
								BOOL bEnsureVisible/*=TRUE*/) const
{
	int nItemCount=GetToolBarCtrl().GetButtonCount();
...
BOOL COXMenuBar::SaveMenuContents(HMENU hMenu, CArchive& ar)
{
	...
	for(int nIndex=0; nIndex<nItemCount; nIndex++)
	{
		...
		sText.ReleaseBuffer();
		ASSERT(!sText.IsEmpty() || (menuItemInfo.fType&MFT_SEPARATOR)!=0);
