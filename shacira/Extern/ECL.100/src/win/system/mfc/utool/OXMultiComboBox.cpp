...
int COXMultiComboBox::GetLBText(int nIndex,int nColIndex, LPTSTR lpszText) const
	...
	if (pRowData != NULL)
		{
		// ... returns the length
		_tcscpy(lpszText,pRowData->GetColumnString(nColIndex).GetBuffer(0));
...
int COXMultiComboBox::FindString(int nStartAfter, int nColIndex,
	...
	int nItemCount = GetCount();
	int nStrLen = _tcslen(lpszString);
...
//	effect : Number of calls to DrawColumnItems are made
	...
	for(int nColIndex=0; nColIndex< m_nColumnCount-1; nColIndex++)
		{
...
LRESULT COXMultiComboBox::OnGetItemData(WPARAM wParam,LPARAM lParam)
	{
	HRESULT result = DefWindowProc(CB_GETITEMDATA, wParam, lParam);
...
// --- Effect : to set the RowData to an Item
	{
	if(DefWindowProc(CB_SETITEMDATA,nIndex,(DWORD)pRowData)==CB_ERR)
		return FALSE;
	else
		return TRUE;
	}
