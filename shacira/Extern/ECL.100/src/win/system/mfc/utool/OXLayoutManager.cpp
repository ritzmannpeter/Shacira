...
BOOL COXLayoutManager::OnSize(int cx, int cy)
{
	...
	// reposition wnd
	int nNumWindows = m_wcTable.GetSize();
	...
	// --- Effect : This function subclasses the container window 
{
	...
	if (m_pfnSuper != NULL)
	{
		...
		if ( (m_hWnd != pContainerWnd->GetSafeHwnd()) || 
		     ((WNDPROC)::GetWindowLong(pContainerWnd->GetSafeHwnd(), GWL_WNDPROC) != GlobalLayoutManagerProc) )
		{
	...
	m_pfnSuper = (WNDPROC)::GetWindowLong(pContainerWnd->GetSafeHwnd(), GWL_WNDPROC);
	::SetWindowLong(m_hWnd, GWL_WNDPROC, (LONG)GlobalLayoutManagerProc);
	...
	//				COXLayoutManager object
{
	...
	ASSERT(m_pfnSuper != NULL);
	ASSERT(m_pfnSuper != GlobalLayoutManagerProc);
	LRESULT result = m_pfnSuper(hWnd, uMsg, wParam, lParam);
