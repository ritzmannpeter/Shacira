...
public:
	...
	inline void MarkAsChanged()
	{
		...
		{
			m_mapChangedMenus.SetAt(GetMenu(),m_mapChangedMenus.GetCount());
		}
	}
...
protected:
	...
	int GetPrevMenuItem(int nItem, BOOL bEnsureVisile=TRUE) const;
	// returns the index of the next (only visible if specified)
	// item in the menu bar
	int GetNextMenuItem(int nItem, BOOL bEnsureVisible=TRUE) const;
...
class OX_CLASS_DECL COXMenuBarHost : public PARENTWND
{
public:
	// --- In  :	
	// --- Out : 
	...
	case WM_CREATE:
		{
			...
			if(lResult!=-1)
			{
				...
				if(!OnCreateMenuBar(m_hDefaultMenu,this,
					WS_CHILD|WS_VISIBLE|CBRS_TOP|CBRS_SIZE_DYNAMIC,AFX_IDW_MENUBAR))
				{
		...
		IsWindowEnabled() && ::GetActiveWindow()==GetSafeHwnd())
	{
		...
		if(!GetMenuBar().IsInDisplayingMode())
		{
				...
				GetMenuBar().IsInSurfingMode()))
			{
				...
				{
					// handle the key 
					if(GetMenuBar().HandleMenuKey(wParam))
						return TRUE;
				}
			}
		}
...
template<class PARENTFRAME, class PARENTDOCKBAR>
class OX_CLASS_DECL COXMenuBarFrame : public COXMenuBarHost<PARENTFRAME>
{
public:
