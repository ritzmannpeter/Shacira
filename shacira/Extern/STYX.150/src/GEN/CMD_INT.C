...
{
  assert0( PT_THISNTM((PT_Term)x, CfgSyms[56]), "cmd expected" );
  if( ! PT_THISPRD((PT_Term)x, PrdSyms[43]) ) return False;
  if( (StdCPtr)x1 != (StdCPtr)NULL )
    *x1 = (cmdSource)GLS_ign_Skip(PT_nthSymbol((PT_Term)x, CfgSyms[37], 1, True));
  return True;
}
...
c_bool cmdSource_root(cmdSource x, GLS_Lst(GLS_Tok)* x1, GLS_Tok* x2,
                      ...
                      cmdOptDoc* x9)
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[37]), "Source expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[39]), "Dfn expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[40]), "OptVal expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[41]), "OptDoc expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[42]), "Val expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[45])
        || PT_THISNTM((PT_Term)x, CfgSyms[46])
        || PT_THISNTM((PT_Term)x, CfgSyms[47]), "Path expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[51]), "OptDrv expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[52]), "Pid expected" );
...
{
  assert0(  PT_THISNTM((PT_Term)x, CfgSyms[55]), "Typ expected" );
