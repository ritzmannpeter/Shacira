...
{ faNode s0   = New(faNode);
  ...
  regset_t res = New(regset_t);
  bug(((unsigned long)dfa_lower) <= ((unsigned long)dfa_upper), 
    "RegSet_Range(%ld,%ld) must be a range",dfa_lower,dfa_upper);
#if defined( _MSDOS ) || defined( _WIN32 )
  res->id     = InterlockedIncrement(&RegSet_TId);
#else
  res->id     = RegSet_TId ++;
...
{
  if (len > 0)
  { RegSet_T a = RegSet_Char((wc_char)(0xffffffff & s[0]));
...
{
  bug0(!(*defining),"allready defining a scanner");
...
{ int i, edgcnt, states = length(RegSet_IMP(scanner)->Nodes);
  short   *StaEdg;
  short   *StaFin;
  wc_char *EdgeC;
  short   *EdgeS;
  ...
  { faNode node = fst(faNode,nodes);
    ...
    { dfaEdge edge = fst(dfaEdge,edges);
      ...
      EdgeC[edgcnt] = (byte)edge->dfa_lower;
...
{ byte cl = (byte)x->nfa_lower,
  ...
  pListEx(x->nfa_toNodes,-1,(void (*)(Abs_T x,StdCPtr any))pSource,scndfn);
  (*prMsg)(")");
}

static void pNfaEdges(List(nfaEdge) x, ScnDfn_T scndfn)
{
  pListEx(x,-1,(void (*)(Abs_T x,StdCPtr any))pNfaEdge,scndfn);
}
...
{ void (*prMsg)(c_string msg) = scndfn->prMsg;
  ...
  (*prMsg)(buf);
  pListEx(x->Source,-1,(void (*)(Abs_T x,StdCPtr any))pSource,scndfn);
  (*prMsg)("\n      ");
  pListEx(x->Edges,-1,(void (*)(Abs_T x,StdCPtr any))pEdge,scndfn);
}
...
{ void (*prMsg)(c_string msg) = CurScnDfn.prMsg;
  ...
  (*prMsg)(buf);
  pListEx(RegSet_IMP(x)->Nodes,0,(void (*)(Abs_T x,StdCPtr any))pNode,&CurScnDfn);
  (*prMsg)("\n");
}
...
{ void (*prMsg)(c_string msg);
  ...
  sprintf(buf,"RegSet_T %d =\n",RegSet_IMP(x)->id);
  (*prMsg)(buf);
  pListEx(RegSet_IMP(x)->Nodes,0,(void (*)(Abs_T x,StdCPtr any))pNode,curdfn);
  (*prMsg)("\n");
}

static Scn_T Scn_dfnEnd_aux(ScnDfn_T scndfn, c_bool diagnose, c_bool* defining)
