...
{ RegSet_T pRes = (RegSet_T)NULL;
  ...
  { int nI;
    pRes = RegSet_Empty();
    for( nI=0; nI < nMax; ++nI )
      pRes = REG_concat(pRes,RegSet_Copy(pRegSet));
    RegSet_Free(pRegSet);
  }
  return pRes;
}
...
{ RegSet_T pRes = (RegSet_T)NULL;
  ...
  { int nI;
    ...
    { RegSet_T pTmp = REG_plus_limited(RegSet_Copy(pRegSet),nI-nMin);
      pTmp = REG_option(pTmp);
      ...
      for(Stream_next(pStream); Stream_ctid(pStream) >= 0; Stream_next(pStream))
      {
        ...
        if (Stream_ctid(pStream) == 1) 
        {
          if( lPatLen != (long*)NULL ) 
          {
            ...
            {
              if( GS_stream_bytepos(pGS,(unsigned int*)&lTmp) != 0 ) break;
        ...
        else 
        {
          ...
          {
            if( GS_stream_bytepos(pGS,(unsigned int*)&lIdx) != 0 ) break;
