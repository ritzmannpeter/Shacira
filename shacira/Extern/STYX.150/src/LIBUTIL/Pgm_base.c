...
{ Scn_Stream cStream = (Scn_Stream)NULL; 
  ...
  { int i;
    ...
    { string s = Scn_tokid(Scn,i);
      Stream_defTokId( cStream, s, (short)i );
      FreeMem(s);
    }
  }
  return cStream;
...
void PGM_initTables(c_string language, PLR_Tab* PTab, Scn_T* Scn)
/* create scan and parse tables according commandline parameter 'language' */
{
  Scn_get(language,Scn);
  *PTab = PLR_getTab(language);
}
...
{ 
  ...
  { Scn_T    Scn = Stream_scn(cStream);
    ...
    if( output )
    {
      s = Scn_tokid(Scn,Stream_ctid(cStream));
