  ...
  while( ! found )
  {
    ...
    { Dir directory = (Dir)NULL; string path;
      ...
      )
      {
        OT_DEL_E(d->DirTab,0);
        if( ! d->skip ) 
          bug(False,"directory '%s' couldn't be opended",path);
        else
...
{ DII_IMP dii = (DII_IMP)NULL; Dir directory = Dir_open(PathName);
  ...
  {
    if( ! skipErr ) bug(False,"directory '%s' couldn't be opended",PathName);
    else            fprintf(STDERR,"directory '%s' couldn't be opended",PathName);
  }
  return (DII) dii;
}
