...
ConcreteImp(GStream_T)  // generic stream type (implementation)
{
  ...
  int      gsd_len; // stream len in bytes
  int      gsd_pos; // current stream position in bytes
  ...
  // fseek function
  // (RC=0 <==> new byte position = origin + offset, RC=-1 <==> error)
  int (*seek)(GStream_T gstream, long offset, int origin);
  // close and free function
...
ConcreteType(CDIConv_T) // ICONV conversion description type
{
  ...
  wc_int  s_endian; // current source big endian byte order
  wc_int  t_endian; // current target big endian byte order
};
...
static wc_string GS_ucs4_to_ucs4_intern(wc_string ws, int size /*bytes*/, wc_int endian)
...
static wc_string GS_ucs4_intern_to_ucs4(wc_string ws, int size /*bytes*/, wc_int endian)
// ucs4 intern <==> ucs4 in big endian order
{
  // assertion: size % wchar_size = 0 !
  if( endian != 0 )
...
static int GS_seek_file(GStream_T gstream, long offset, int origin)
...
static int GS_seek_string(GStream_T gstream, long offset, int origin)
