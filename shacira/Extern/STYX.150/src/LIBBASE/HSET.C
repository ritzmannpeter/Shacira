...
#define HS__TYP_LEN(typ)      ( ((int)(*(typ))) + 1 )
#define HS__TYP_ARITY(typ)    ( ((int)(*(typ))) >> 1 )
...
/* tuple type of rel ( expects relation, produces error on set ) */
{
  ...
  (
    ...
    : ( HS__SETTYP(rel) == HS_T_NREL )
      ? HS__IMP(rel)->set.nrel.tpltyp : ( BUG, (StdCPtr*)NULL )
  );
}
...
{ int argcnt = ( HS__TYP_ARITY(l) + HS__TYP_ARITY(r) ) * 2, i, j;
  ...
  HS__TYP_COL(typ,0) = (StdCPtr)argcnt;
...
{ StdCPtr* ltyp = getTplTyp(l); StdCPtr* rtyp = getTplTyp(r); 
  int cnt = (int)cols[0], i, j;

  for( i=1, j=2; i < cnt; i+=2, j+=2 )
    if( (int)cols[i] < 1 || (int)cols[i] > HS__TYP_ARITY(ltyp)               ||
        (int)cols[j] < 1 || (int)cols[j] > HS__TYP_ARITY(rtyp)               || 
        HS__TYP_EQUAL(ltyp,(int)cols[i]) != HS__TYP_EQUAL(rtyp,(int)cols[j]) ||
        HS__TYP_HASH(ltyp,(int)cols[i])  != HS__TYP_HASH(rtyp,(int)cols[j])   )
      return( False );
  return( True );
}
...
{ StdCPtr* typ = HS__TPL_TYP(tpl);
  int arity = HS__TYP_ARITY(typ), cols = (int)domvals[0],
...
{ StdCPtr* ltyp = HS__TPL_TYP(l); int cnt = (int)cols[0], i, j;

  for( i=1, j=2; i < cnt; i+=2, j+=2 )
  { bool (*equal)(HS_Dom l,HS_Dom r) = HS__TYP_EQUAL(ltyp,(int)cols[i]);

    if( ! (*equal)(HS__TPL_DOM(l,(int)cols[i]),HS__TPL_DOM(r,(int)cols[j])) ) 
      return( False );
  }
  return( True );
}
...
{ HSSET* rel; MAPTY rmaptyp; int cnt;
  ...
  BUG_HS_RELARGCNT(cnt = (int)arglst[0]);
...
{ StdCPtr* typ = getTplTyp(rel); HS_Set dst;
  int cols = (int)domvals[0], arity = HS__TYP_ARITY(typ);
...
#define HS_SET_ELM(elm,set) HS_setElm((HS_Elm)(elm),set)
/* set = set U { elm } */
{
  BUG_NULL(set);
