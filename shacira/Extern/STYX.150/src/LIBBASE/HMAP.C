...
/* */
{
  bug0( *slot == NULL, "attempt to add an element which is allready a member");
  *slot = New( HMP_Ent );
  (*slot)->next = NULL;
...
{ long     h = (*m->T->domhsh)(d);
  HMP_Ent *x = EntryLoc(m,d,h);
  bug0(*x == (HMP_Ent) NULL,"allready defined");
  addEntry(x,m,d,h,r);
  addTableSlot(m);
 m->Version += 1;
}
...
#define MAP_forItrAsg(DomVar,RngVar,ItrVar,MapExpr)             \
        for (ItrVar = MAP_newItr(MapExpr);                      \
             MAP_emptyItr(ItrVar)                               \
             ? (MAP_freeItr(ItrVar),         False)             \
             : (MAP_getItrAsg(ItrVar, &DomVar, &RngVar), True );\
            )
