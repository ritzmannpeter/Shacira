...
*/
{ StdCPtr* arglst; int argcnt = va_arg(args,int), i;
  arglst = (StdCPtr*)NewMem((argcnt+1)*sizeof(StdCPtr));
  arglst[0] = (StdCPtr)argcnt;
  for( i=0; i < argcnt; ++i ) arglst[i+1] = va_arg(args,StdCPtr);
  return( arglst );
}
...
#if defined( _MSDOS ) || defined( _WIN32 )
#ifdef __BORLANDC__
#define PUSH_FUN_ARG1(arg) __asm mov eax, arg
#define PUSH_FUN_ARG2(arg) __asm push eax 
#else
