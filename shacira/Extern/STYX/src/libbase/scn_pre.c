...
{ MAPTY pMapTyp = MAP_newTyp
  ...
  pPP->szCurMacSep[0] = pPP->szCurMacSep[1] = pPP->szCurMacSep[2] = '\0';
...
{ string szS = cTokVal, szT, szID;
  symbol pID;
  while( (*szS != 0) && !SPP_ISSPACE(*szS) ) szS++;
  while( (*szS != 0) &&  SPP_ISSPACE(*szS) ) szS++;
...
static symbol SPP_applyMacro(SPP_T pPP, symbol pTokVal, symbol* pMacNam)
...
{ SPP_T pPP  = (SPP_T)Stream_get_ctxval(pStream,SPP_premac);
    ...
    if( !strcmp(cTokVal,SPP_PREPAR(pPP,SPP_PAT_ELSE)) )
    {
      ...
      { long dmy = 0;
        if( OT_TOP(long,pPP->pCondRow) == SPP_COND_SKIP )
          dmy = OT_UPD(long,pPP->pCondRow,lCnt-1,SPP_COND_SCAN);
        else 
        if( OT_TOP(long,pPP->pCondRow) == SPP_COND_SCAN )
          dmy = OT_UPD(long,pPP->pCondRow,lCnt-1,SPP_COND_SKIP);
      }
    }
    else
  ...
  else
  {
    ...
    { symbol pValSym    = stringToSymbol(cTokVal), 
      ...
      { 
        *cMacVal = SPP_applyMacro(pPP,pLowValSym,&pMacNam);
          ...
          ( pPP->pCurMacRow == (ROW(symbol))NULL || strcmp(cTokVal,pPP->szCurMacSep) )*/ )
      {
        ...
        if( pPP->pCurMacRow != (ROW(symbol))NULL &&
            strlen(pPP->szCurMacSep) == 0 && !Sink_empty(pPP->pCurMacPar) ) 
