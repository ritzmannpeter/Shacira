...
{ OtObjVec objects = OT__OBJECTS(tab); 
  ...
  { 
    ...
    OT__OBJECTS(tab) = 
      (OtObjVec)ResizeMem(OT__OBJECTS(tab),OT__SIZE(tab)*sizeof(OtObj));
#else
    INT i;
...
#define OT_L_FIND_EQ(tab,obj)  \
        OT_l_find(tab,ABS_CAST(OT_Obj,obj),(bool (*)(OT_Obj tobj, OT_Obj Obj))NULL)
#define OT_L_FIND(tab,obj,cmp) \
        OT_l_find(tab,ABS_CAST(OT_Obj,obj),(bool (*)(OT_Obj tobj, OT_Obj Obj)) cmp)
...
{ OT_Obj center; OT_Obj tmp; INT left, right, centeri;
  ...
  while( rows > 1 )
  {
    center  = OT__OBJECT(tab,ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,rows / 2)));
    centeri = ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,rows / 2));
    left    = 0;
    right   = rows - 1;
    while( left <= right )
    {
      ...
      ( 
        OT_lex
        (
          (*cmp3)(OT__OBJECT(tab,ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,left))),center),
          ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,left))-centeri
        ) < 0 
      ) ++left;
      while
      ( 
        OT_lex
        (
          (*cmp3)(OT__OBJECT(tab,ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,right))),center),
          ABS_CAST(OtIdx,OVEC__ELEMENT(idxvec,right))-centeri
        ) > 0 
      ) --right;
      if( left <= right )
      {
...
{ OT_Tab idxtab; INT i;
  ...
  if( OT__CNT(tab) )
  {
    for( i=0; i < OT__CNT(tab); ++i ) OT_t_ins(idxtab,ABS_CAST(OT_Obj,i));
...
{ INT left, right, center, res;
  ...
  while( left < right )
  {
    ...
    res = ( idxtab != (OT_Tab)NULL )
          ? (*cmp3)(OT__OBJECT(tab,ABS_CAST(OtIdx,OT__OBJECT(idxtab,center))),obj)
  ...
  res = ( idxtab != (OT_Tab)NULL )
        ? (*cmp3)(OT__OBJECT(tab,ABS_CAST(OtIdx,OT__OBJECT(idxtab,left))),obj)
...
{ INT i;
  ...
  for( i=0; i < OT__CNT(tab); ++i )
  {
    ...
    if( (StdCPtr)idxtab != (StdCPtr)NULL )
      (*pMbr)( OT__OBJECT(tab,ABS_CAST(OtIdx,OT__OBJECT(idxtab,i))) );
    else
...
{ va_list args; StdCPtr* arglst; OT_Obj* objs; INT i, j;
  ...
  va_start(args,tab); 
  arglst = MakeArgLst(args,4,argcnt,fun,any,tab); 
  va_end(args);
  BUG_OT__ACNT(argcnt);
