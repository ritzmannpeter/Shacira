...
{ long len,i;
  getLong(&len);
  *v = (wc_string) NewMem((len+1) * sizeof(wc_char));
  for(i = 0; i < len; i++)
    getULong((unsigned long*)&(*v)[i]);
  (*v)[len] = WCHAR_LITERAL('\0');
}
...
{ long len,i;
  ...
  *v = (wc_string) NewMem((len+1) * sizeof(wc_char));
  for(i = 0; i < len; i++)
    fgetULong(img,(unsigned long*)&(*v)[i]);
  (*v)[len] = WCHAR_LITERAL('\0');
}
...
{ long getid;
  ...
  { OT_Obj dmy = NULL;
    ...
    getData(v);
    dmy = OT_UPD(Abs_T,pcimg->getmap,getid,*v);
  }
}
...
{ long getid;
  ...
  { OT_Obj dmy = NULL;
    ...
    getData(img,v);
    dmy = OT_UPD(Abs_T,img->getmap,getid,*v);
  }
}
